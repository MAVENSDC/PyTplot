...
times = pt.data_quants[SpecIN].data.index.values
data = pt.data_quants[SpecIN].data.values
bins = pt.data_quants[SpecIN].spec_bins.values

asdf = xr.DataArray(data, dims=['x', 'y'])
asdf.coords['time'] = ('x', times)
asdf.coords['spec_bins'] = (('x', 'y'), bins)